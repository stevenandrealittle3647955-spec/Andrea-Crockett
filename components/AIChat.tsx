...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'The stars are aligning... 🌌 Ask me about your destiny or our readers.' }
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex flex-col items-end pointer-events-auto">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[90vw] md:w-96 bg-[#1a103c]/90 backdrop-blur-xl border border-fuchsia-500/20 rounded-2xl overflow-hidden shadow-2xl shadow-fuchsia-500/10"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-[#4c1d95]/50 to-[#831843]/50 p-4 flex justify-between items-center border-b border-white/10">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-fuchsia-300 animate-pulse" />
                <h3 className="font-heading font-bold text-white tracking-wider">CELESTIA AI</h3>
              </div>
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-[#7c3aed] p-2 rounded-lg hover:bg-[#6d28d9] transition-colors disabled:opacity-50"
                  data-hover="true"
                >
      ...
      <motion.button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-tr from-[#7c3aed] to-[#db2777] flex items-center justify-center shadow-lg shadow-fuchsia-500/30 border border-white/20 z-50 group"
        data-hover="true"
      >
        {isOpen ? (
